...
import { JSONResponse, User, Session } from "~~/appopse/misc/types";
import { getUserSession } from "~~/appopse/misc/utils/sessions";
import { hasPermission } from "~~/appopse/autha/permissions";
...
export async function create(event: H3Event): Promise<JSONResponse> {
  // Return error because all users will be created from /autha/register endpoint
  const response = {} as JSONResponse;
  response.status = "fail";
  response.error = createError({
    statusCode: 422,
    statusMessage: "All users must be created from autha/register endpoint",
  });
