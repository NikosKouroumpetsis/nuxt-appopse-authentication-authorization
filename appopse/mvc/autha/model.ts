...
import { generateNewPassword } from "~~/appopse/misc/utils/passwords";
import { getUserSession } from "~~/appopse/misc/utils/sessions";
import {
  createGoogleUser,
...
import {
  ...
  verifyGoogleToken,
} from "~~/appopse/misc/utils/logins";
...
export async function login(event: H3Event): Promise<JSONResponse> {
  ...
  if (platform === "app") {
    ...
    setHeader(event, "appopse-refresh-token", "Bearer " + tokens.refreshToken);
  ...
  if (platform === "browser") {
    ...
    setCookie(event, "appopse-refresh-token", "Bearer " + tokens.refreshToken, {
      httpOnly: true,
      secure: true,
...
export async function profile(event: H3Event): Promise<JSONResponse> {
  ...
  const sessionId = getCookie(event, "appopse-sid");
