...
import { TokensSession, User } from "~~/appopse/misc/types";
...
import {
  ...
  validateUserLogin,
  validateEmail,
} from "~~/appopse/misc/utils/validators";
import { hashPassword, makeUuid } from "~~/appopse/misc/utils/passwords";
import {
  getUserByEmail,
  getUserByUuid,
  updateUserProfile,
} from "~~/appopse/misc/utils/users";
import { login, logout } from "~~/appopse/misc/utils/logins";
import {
  getNewTokens,
  deactivateRefreshTokens,
  verifyAccessToken,
} from "~~/appopse/misc/utils/tokens";
import { sendVerifyEmail, sendResetEmail } from "~~/appopse/misc/utils/emails";
import { deactivateUserSessions } from "~~/appopse/misc/utils/sessions";
...
export async function isAuthenticated(
  ...
  if (platform === "app")
    ...
    accessToken = event.node.req.headers["appopse-access-token"] as string;
  else if (["browser", "browser-dev"].includes(platform)) {
    accessToken = getCookie(event, "appopse-access-token") as string;
  }
  ...
  if (errorOrUser instanceof jwt.TokenExpiredError) {
    ...
    if (platform === "app") {
      setHeader(event, "appopse-access-token", "Bearer " + tokens.accessToken);
      setHeader(
        event,
        "appopse-refresh-token",
        "Bearer " + tokens.refreshToken
      );
      if (tokens.sid) setHeader(event, "appopse-sid", tokens.sid);
    }

    // If platform is browser production, set tokens in secure, httpOnly cookies
    if (platform === "browser") {
      setCookie(event, "appopse-access-token", "Bearer " + tokens.accessToken, {
        httpOnly: true,
        secure: true,
      });
      ...
      // Set session id
      if (tokens.sid) setCookie(event, "appopse-sid", tokens.sid);
    }
...
export async function deleteAccount(
  ...
  if (!nullOrUser) {
    console.log(
      "User to delete from useAppopse not found. This should not happen."
    );
    return createError({
      statusCode: 500,
  ...
  deleteCookie(event, "appopse-access-token");
  deleteCookie(event, "appopse-refresh-token");
  deleteCookie(event, "appopse-sid");
...
export async function resetPassword(event: H3Event): Promise<H3Error | true> {
  ...
  const resetToken = jwt.sign(resetUser, config.appopseResetTokenSecret, {
    expiresIn: "1h",
    issuer: "NuxtAppopse",
    jwtid: makeUuid(),
  });
...
export async function verifyUserEmail(event: H3Event): Promise<H3Error | true> {
  ...
  const emailVerifyToken = jwt.sign(
    verifyUser,
    config.appopseVerifyTokenSecret,
    {
      expiresIn: "24h",
      issuer: "NuxtAppopse",
      jwtid: makeUuid(),
