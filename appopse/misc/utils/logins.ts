...
export async function getTokensAfterGoogleLogin(
  ...
  // Create refresh token
  const tokenId = makeUuid();
  const refreshToken = jwt.sign(publicUser, config.appopseRefreshTokenSecret, {
    expiresIn: "14d",
    issuer: "NuxtAppopse",
    jwtid: tokenId,
  });
...
export async function verifyGoogleToken(
  ...
  const clientId = useRuntimeConfig().appopseGoogleClientId;
...
export function getAuthenticatedRoutes(): Array<string> {
  return ["/api/appopse/users", "/api/appopse/refresh-tokens"];
}
