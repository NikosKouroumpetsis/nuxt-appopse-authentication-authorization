...
import { User, TokensSession, Session } from "~~/appopse/misc/types";
...
export async function getNewTokens(
  ...
  if (platform === "app")
    ...
    refreshToken = event.node.req.headers["appopse-refresh-token"] as string;
  else if (["browser", "browser-dev"].includes(platform))
    refreshToken = getCookie(event, "appopse-refresh-token") as string;
...
export function verifyAccessToken(token: string): H3Error | JwtPayload {
  ...
  jwt.verify(token, config.appopseAccessTokenSecret, (err, user) => {
    if (err) {
      console.log(err);
...
export function verifyPasswordResetToken(token: string): H3Error | JwtPayload {
  ...
  jwt.verify(token, config.appopseResetTokenSecret, (err, user) => {
    if (err) {
      console.log(err);
...
export function verifyEmailVerificationToken(
  ...
  jwt.verify(token, config.appopseVerifyTokenSecret, (err, user) => {
    if (err) {
      console.log(err);
...
export async function createNewTokensFromRefresh(
  ...
  if (user) {
    ...
    const accessToken = jwt.sign(publicUser, config.appopseAccessTokenSecret, {
      expiresIn: "15m",
    });
    ...
    const refreshToken = jwt.sign(
      publicUser,
      config.appopseRefreshTokenSecret,
      {
        expiresIn: "14d",
        issuer: "NuxtAppopse",
...
export async function verifyRefreshToken(
  ...
  jwt.verify(token, config.appopseRefreshTokenSecret, async (err, token) => {
    if (err) {
      console.log(err);
      error = createError({
        statusCode: 403,
  ...
  if (verifiedTokenPayload) {
    ...
    if (verifiedTokenPayload.iss !== "NuxtAppopse") {
...
export function getTokenPayload(
  ...
  switch (type) {
    case "access":
      tokenSecret = config.appopseAccessTokenSecret;
      break;
    case "refresh":
      tokenSecret = config.appopseRefreshTokenSecret;
      break;
    case "reset":
      tokenSecret = config.appopseResetTokenSecret;
      break;
  }
