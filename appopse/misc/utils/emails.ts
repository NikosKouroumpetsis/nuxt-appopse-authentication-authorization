import { User, EmailOptions } from "~~/appopse/misc/types";
...
export async function sendResetEmail(
  ...
  // Get emailer and url
  const emailer = config.appopseEmailer;
  const url = config.appopsePublicUrl;

  // nodemailer-service
  const service = config.appopseNodemailerService;
  const serviceSender = config.appopseNodemailerServiceSender;
  const servicePassword = config.appopseNodemailerServicePassword;

  // nodemailer-smtp
  const smtpHost = config.appopseNodemailerSmtpHost;
  const smtpPort = config.appopseNodemailerSmtpPort;
  const smtpSender = config.appopseNodemailerSmtpSender;
  const smtpPassword = config.appopseNodemailerSmtpPassword;
  ...
  const options = {
    to: user.email,
    subject: "Nuxt APPOPSE reset password link",
    text: `
    ...
    Password reset link: ${url}/appopse/verify?token=${token}
    `,
    html: `
    ...
    <p>Password reset link: ${url}/appopse/verify?token=${token}</p>`,
  } as EmailOptions;
  ...
  if (emailer === "sendgrid") {
    ...
    sendgridOptions.from = config.appopseSendgridSender;
...
export async function sendVerifyEmail(
  ...
  const options = {
    to: user.email,
    subject: "Nuxt APPOPSE please verify your email",
    text: `
    Hello ${user.first_name},
    ...
    Follow the link to verify your email: ${url}/appopse/verifyemail?token=${token}
    `,
    html: `
    <p>Hello ${user.first_name}</p>,
    ...
    <p>Follow the link to verify your email: ${url}/appopse/verifyemail?token=${token}</p>`,
  } as EmailOptions;
...
export async function emailWithSendgrid(
  ...
  const apiKey = config.appopseSendGridApiKey;
  let emailError = null;
