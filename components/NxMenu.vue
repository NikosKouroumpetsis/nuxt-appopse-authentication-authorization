<template>
  <ul v-if="menu" class="menu" :class="[shadowClass]">
    <li
      ...
      class="menu-item"
      :class="[borderBottomClass(item), showClass(item), disabledClass(item)]"
      @click.stop="clicked(item)"
    >
      {{ item.name }}
      <span v-if="item.children && item.showChildren !== false">
        <span class="arrow" @click="toggleShowChildren(showChildren)"
          >&raquo;</span
        >
        <nxMenu
          v-if="showChildren"
          :menu="item.children"
          :shadow="shadowOnChildren"
        />
      </span>
    </li>
  </ul>
...
import { NxLink } from "~~/appopse/misc/types";
