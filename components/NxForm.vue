  ...
  <form class="mb-5" :style="`width: ${width}rem;`">
    <div class="mb-3">
      <div v-for="(item, index) in data" :key="index">
        <label
          v-if="item.label && item.show !== false"
          :for="item.label"
          class="form-label"
          ><strong>{{ item.label }}</strong></label
        >
        <input
          v-if="item.type === 'input:text' && item.show !== false"
          type="text"
        ...
        />
        <input
          v-if="item.type === 'input:password' && item.show !== false"
          type="password"
          class="form-control mb-3"
          :id="item.id"
          :disabled="item.disabled ?? false"
        ...
        />
        <input
          v-if="item.type === 'input:number' && item.show !== false"
          type="number"
          class="form-control mb-3"
          :id="item.id"
          :disabled="item.disabled ?? false"
        ...
        />
        <input
          v-if="item.type === 'input:email' && item.show !== false"
          type="email"
          class="form-control mb-3"
          :id="item.id"
          :disabled="item.disabled ?? false"
        ...
        />
        <textarea
          v-if="item.type === 'textarea' && item.show !== false"
          :id="item.id"
          :disabled="item.disabled ?? false"
          :value="item.value ?? ''"
          @change="updateInputs"
...
import { NxFormInput } from "~~/appopse/misc/types";
