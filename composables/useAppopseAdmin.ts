...
export default function useAppopseAdmin() {
  return {
    getUsers,
    createUser,
    updateUser,
...
async function getUsers(): Promise<JSONResponse> {
  const response = await $fetch("/api/appopse/users", {
    headers: {
...
async function updateUser(user: User): Promise<JSONResponse> {
  const response = await $fetch(`/api/appopse/users/${user.uuid}`, {
    method: "PUT",
    headers: {
      "client-platform": "browser",
    },
...
async function userHasPermission(
  ...
  const response = await $fetch(
    `/api/appopse/users/${user.uuid}/permission/${permission}`,
    {
      headers: {
        "client-platform": "browser",
      },
...
async function getRefreshTokens(): Promise<JSONResponse> {
  const response = await $fetch("/api/appopse/refresh-tokens", {
    headers: {
      "client-platform": "browser",
    },
  });
...
async function deleteRefreshToken(
  ...
  const response = await $fetch(`/api/appopse/refresh-tokens/${id}`, {
    method: "DELETE",
    headers: {
      "client-platform": "browser",
    },
