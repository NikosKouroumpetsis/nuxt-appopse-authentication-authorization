<template>
  <div>
    <div>
      ...
      <p>
        Understanding the following concepts will help you work with Nuxt
        appopse faster.
      </p>
      <h2 id="both-backend-and-frontend">Both Backend and Frontend</h2>
      <p>
        Nuxt Appopse is both frontend and backend . The main authentication and
        authorization logic takes place in the backend. Nuxt appopse adds
      ...
      <p>
        Nuxt appopse uses Nuxt&#39;s server engine
        <a href="https://nuxt.com/docs/guide/concepts/server-engine">Nitro</a>
        and the entire backend is built on it. Nuxt appopse adds the following
        directories to your <strong>server/api</strong> directory.
      </p>
      <ul>
        <li><strong>appopse/autha</strong>: authentication handler</li>
        <li><strong>appopse/refresh-tokens</strong>: refresh tokens handler</li>
        <li><strong>appopse/users</strong>: global users handler</li>
        <li>
          <strong>appopse/gadgets</strong>: an example api handler that you can
          use to copy and paste and quickly create api handlers of your own
        </li>
      </ul>
      <h2 id="backend-for-frontend">Backend for Frontend</h2>
      <p>
        Nuxt appopse uses the Backend For Frontend (BFF) architectural pattern
        to increase the security of your Nuxt application. A BFF pattern allows
        Nuxt appopse to provide the best security practices for any client. Nuxt
        appopse differentiates between two types of clients:
      ...
      <p>
        ...
        it must be sent with every request. Client platform allows Nuxt appopse
        to provide the best practices for securing your app.
      ...
      <p>
        Nuxt appopse uses signed JSON web tokens (JWT) as part of its security.
      ...
      <p>
        ...
        combination. If successful, Nuxt appopse will create an access token and
        refresh token and send them back to the client.
      </p>
      <p>
        If the client is an <strong>app</strong>, the tokens will be sent in the
        header as <strong>appopse-access-token</strong> for the access token,
        and <strong>appopse-refresh-token</strong> for the refresh token.
      </p>
      <p>
      ...
      <p>
        ...
        token. If you use the pages provided by Nuxt appopse, your access and
        refresh tokens will be automatically replaced once Nuxt appopse detects
      ...
      <p>
        ...
        <code>/api/appopse/autha/refresh</code> with an unexpired refresh token.
        If your refresh token has expired, you will not be able to get a new set
        of tokens and you&#39;ll need to login.
      </p>
      <p>
      ...
      <p>
        ...
        <code>browser-only</code>, Nuxt appopse will automatically refresh your
      ...
      <p>
        Nuxt appopse keeps track of expired refresh tokens. Let&#39;s say you
        you have a one refresh token in the database. If you refresh your
        tokens, you get a new set of tokens, and the old refresh token will be
      ...
      <p>
        Nuxt appopse uses sessions to manage user sessions. Every user can only
        have one session at a time.
      </p>
    </div>
  </div>
</template>
